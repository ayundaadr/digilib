<?php
	...
	$db=new MySQL();
	$db->connect();
	$db->execute("SELECT kodeDosen, username, password, nama, email, dateInput, dateUpdate, tempatLahir, tanggalLahir, alamat, image
		FROM tb_dosen
		limit $posisi, $batas"); 
	$data = $db->get_dataset();
	if(count($data) > 0) {
	  ...
	  ?>
		<tr>
		  ...
		  <td>
			...
			<a href="?page=process&o=d&id=<?=$data[$i][0]?>&tud=1" rel="tooltip" title="Hapus" class="btn btn-mini btn-danger" onClick="return confirm('Apakah anda yakin ingin menghapus Pelajaran ini?"><i class="icon-remove icon-white"></i></a>
		  </td>
		</tr>
	  <?php } ?>
	  </tbody>
...
<?php } else { ?>
<div class="row-fluid">
	<h2 class="title title-large">Daftar Dosen</h2>
	<hr />
	<div class="alert alert-error">
	  <strong>Warning!</strong> Daftar dosen kosong
	</div>
</div>
<?php } ?>
<hr />
<?php	
	...
	for($j=1;$j<=$jhal;$j++){
		if($j != $hal) {
			echo "<li><a href=\"?page=daftar_dosen&hal=$j\">$j</a></li>";
		}else{
...
<div class="modal hide fade" id="modalEdit<?=$i?>" tabindex="-1" role="dialog" aria-labelledby="modalEdit<?=$i?>Label" aria-hidden="true">
  ...
  <div class="modal-body">
      <div class="row-fluid">
		<input type="hidden" name="kodeDosen" id="kodeDosen" value="<?=$data[$i][0]?>">
...
<div class="modal hide fade" id="modalTambah" tabindex="-1" role="dialog" aria-labelledby="modalTambahLabel" aria-hidden="true">
  ...
  <div class="modal-body">
      <div class="row-fluid">
		<input type="hidden" name="kodeDosen" id="kodeDosen" value="<?=$idUser?>">
		<input type="hidden" name="tud" id="tud">
		<div>
			<label><p class="title">Nama</p>
